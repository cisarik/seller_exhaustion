...
def apply_coach_recommendations(
    ...
    print(f"[PARAMS ] 🔧 Applying {len(analysis.recommendations)} recommendations...")
        ...
        try:
            if rec.category == RecommendationCategory.FITNESS_WEIGHTS:
                ...
                if hasattr(new_fitness, rec.parameter):
                    ...
                    print(f"[PARAMS ] ✅ fitness.{rec.parameter}: {old_val} → {rec.suggested_value}")
                    applied_count += 1
                ...
                if hasattr(new_fitness, rec.parameter):
                    ...
                    print(f"[PARAMS ] ✅ fitness.{rec.parameter}: {old_val:.2f} → {rec.suggested_value:.2f}")
                    applied_count += 1
                ...
                if rec.parameter == "fitness_function_type":
                    ...
                    print(f"[PARAMS ] ✅ fitness.fitness_function_type: '{old_val}' → '{rec.suggested_value}'")
                    applied_count += 1
                ...
                if hasattr(new_ga, rec.parameter):
                    ...
                    print(f"[PARAMS ] ✅ ga.{rec.parameter}: {old_val} → {rec.suggested_value}")
                    applied_count += 1
                ...
                if rec.parameter == "bounds" and isinstance(rec.suggested_value, dict):
                    ...
                    print(f"[PARAMS ] ✅ bounds: Override for {len(rec.suggested_value)} parameters")
                    applied_count += 1
        
        except Exception as e:
            print(f"⚠️  Failed to apply recommendation {rec.parameter}: {e}")
            print(f"[PARAMS ] ⚠️  Failed: {rec.parameter} - {e}")
    
    print(f"\n📊 Applied {applied_count}/{len(analysis.recommendations)} recommendations")
    print(f"[PARAMS ] 📊 Applied {applied_count}/{len(analysis.recommendations)} recommendations")
