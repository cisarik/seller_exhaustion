...
**Tech:** Python 3.10, async I/O, PySide6 UI, PyQtGraph candles, Polygon.io crypto REST, pandas vectorized feature engine, optional Yahoo/yfinance fallback  
**Theme:** Dark Forest UI (QSS)  
**Owner:** Michal  
**Version:** 2.2 (CPU Simplification)

**IMPORTANT**: This PRD describes the BACKTESTING application.  
For live trading agent specifications, see **PRD_TRADING_AGENT.md**.

> **Note (v2.2, January 2025):** GPU/Spectre pipeline removed after profiling (319× slower and incorrect). The system now uses pandas vectorized feature computation with CPU-only backtesting/optimization (~0.16 s feature builds, ~0.18 s backtests, ~4–5 s generations on a 12-core CPU).
...
- Distribuovaná optimalizácia; GPU akcelerácia (experiment so Spectre odstránený, CPU multicore plne postačuje).
...
### Acceleration Model (v2.2 – Simplified)
- **CPU-only pipeline:** Feature computation relies on pandas vectorized operations; Spectre/GPU code removed after profiling at 319× slower with incorrect signals.
- **Performance snapshot:** Feature build 0.16 s, backtest 0.18 s, GA generation ~4–5 s on 12-core CPU (~30 s single-core), 100 generations ≈ 8 min.
- **Why no GPU:** Dataset size (<10 K bars) favors CPU cache; GPU path added 900+ lines of maintenance burden without benefit.
- **Lesson learned:** Profile before optimizing; correctness + simplicity beat premature GPU work.
...
### 6. Acceleration
Feature computation uses pandas vectorized operations only (Spectre/GPU path removed in v2.2 after failing performance/correctness checks). Optimization supports single-core and multi-core CPU evaluation, delivering ~4–5 s generations on a 12-core CPU (~30 s single-core).
...
- `docs/STRATEGY_DEFAULTS_GUIDE.md` - Default behavior and customization guidance
- `docs/STRATEGY_EXPORT_GUIDE.md` - Export/import workflow
